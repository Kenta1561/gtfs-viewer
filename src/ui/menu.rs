...
fn get_search_field(app: &App) -> Paragraph {
    let text = Text::from(app.station.trigger.borrow());  //todo right func?
    Paragraph::new(text)
        .block(Block::default()
            .borders(Borders::ALL)
...
fn render_station_list<B>(
    ...
    where B: Backend
{
    if app.station.changed {
        app.station.widget.set_items(db.fetch_stations(&app.station.trigger).unwrap());
        app.station.changed = false;
    }

    let items: Vec<ListItem> = app.station.widget.items.iter()
    ...
    frame.render_stateful_widget(list, area, &mut app.station.widget.state);
}
